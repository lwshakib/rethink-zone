...
export const exportToPNG = async (
  ...
  const themeStroke = theme === "dark" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)";
  ...
  const drawAll = () => {
    // 1. Groups (Figures)
    (shapes.figures || []).forEach(f => drawFigure(ctx, f, themeStroke, themeText, "rgba(255,255,255,0.02)", 1));
    
    // 2. Rectangles
    (shapes.rectangles || []).forEach(r => drawRect(ctx, r, themeStroke, 1));
    
    // 3. Images
    (shapes.images || []).forEach(im => drawImageItem(ctx, im, imageCache, 1));
...
export const exportToSVG = (
  ...
  (shapes.rectangles || []).forEach(r => {
    const dash = r.strokeDashArray ? `stroke-dasharray="${r.strokeDashArray.join(',')}"` : "";
  ...
  (shapes.texts || []).forEach(t => {
    const x = t.textAlign === "center" ? t.x + t.width / 2 : t.textAlign === "right" ? t.x + t.width : t.x;
