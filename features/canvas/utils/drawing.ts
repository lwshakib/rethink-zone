...
export const drawRect = (
  ...
  if (sw > 0) {
    ...
    if (r.strokeDashArray)
      ctx.setLineDash(r.strokeDashArray.map((v) => v / zoom));
...
export const drawCircle = (
  ...
  if (sw > 0) {
    ...
    if (c.strokeDashArray)
      ctx.setLineDash(c.strokeDashArray.map((v) => v / zoom));
    ctx.stroke();
  }
  ctx.restore();
...
export const drawFrame = (
  ...
  if (f.deviceType === "phone" || f.deviceType === "tablet") {
    ...
    if (isBrowser) {
      ...
      const bx = x + 60 / zoom;
      ctx.beginPath();
      ctx.moveTo(bx + 10 / zoom, iconY);
      ctx.lineTo(bx, iconY);
      ctx.lineTo(bx + 4 / zoom, iconY - 4 / zoom);
      ctx.moveTo(bx, iconY);
      ctx.lineTo(bx + 4 / zoom, iconY + 4 / zoom);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(fx, iconY);
      ctx.lineTo(fx + 10 / zoom, iconY);
      ctx.lineTo(fx + 6 / zoom, iconY - 4 / zoom);
      ctx.moveTo(fx + 10 / zoom, iconY);
      ctx.lineTo(fx + 6 / zoom, iconY + 4 / zoom);
      ctx.stroke();
  ...
  if (isSelected) {
    ...
    ctx.fillText(
      `Frame ${f.frameNumber}${f.deviceType ? ` (${f.deviceType})` : ""}`,
      f.x,
      f.y - 4 / zoom
    );
  }
...
export const drawPoly = (
  ...
  if (p.type === "Triangle") {
    ...
    const ry =
      p.type === "Cylinder" ? Math.min(h * 0.15, 20) : Math.min(h * 0.1, 15);
    const cx = p.x + rx;
    ...
    if (sw > 0) {
      ...
      if (p.strokeDashArray)
        ctx.setLineDash(p.strokeDashArray.map((v) => v / zoom));
      ctx.beginPath();
      ctx.moveTo(p.x, p.y + ry);
      ctx.lineTo(p.x, p.y + h - ry);
      ctx.ellipse(cx, p.y + h - ry, rx, ry, 0, 0, Math.PI, false);
...
export const drawLine = (
  ...
  if (sw > 0) {
    ...
    if (l.strokeDashArray)
      ctx.setLineDash(l.strokeDashArray.map((v) => v / zoom));
    ctx.stroke();
  }
  ctx.restore();
...
export const drawArrow = (
  ...
  if (a.strokeDashArray)
    ctx.setLineDash(a.strokeDashArray.map((v) => v / zoom));
  ...
  if (isSelected) {
    ...
    ctx.lineWidth = 1.5 / zoom;
    ctx.beginPath();
    ctx.arc(a.x1, a.y1, hs / 2, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(a.x2, a.y2, hs / 2, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
  }
};
