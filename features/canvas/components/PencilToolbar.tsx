...
const AdvancedColorPicker = ({
  ...
  const [hsv, setHsv] = useState(() =>
    hexToHsv(color.startsWith("#") ? color : "#ffffff")
  );
  ...
  const handleSatValueChange = useCallback(
  ...
  return (
    <div className="flex flex-col gap-3 p-1">
      ...
      <div className="flex items-center gap-2 bg-muted/50 border border-border rounded-sm px-2 py-1.5 mt-1">
        <div
          className="w-5 h-5 rounded-sm shadow-sm border border-white/10"
    ...
    >
      ...
      <div className="relative px-1 border-r border-border/50">
        ...
        >
          ...
          />
          <ChevronDown
            className={`h-3 w-3 text-muted-foreground transition-transform duration-200 ${activePopover === "color" ? "rotate-180" : ""}`}
          />
        </button>
