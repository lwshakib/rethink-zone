...
import { 
  RectShape, CircleShape, LineShape, PathShape, 
  ImageShape, TextShape, FrameShape, PolyShape, 
  FigureShape, CodeShape, ShapeCollection, ArrowShape 
} from "../types";
...
const MiniMap: React.FC<MiniMapProps> = ({
  rectangles, circles, lines, arrows, paths, images, texts, frames, polygons, figures, codes,
  pan, zoom, containerWidth, containerHeight, onPanChange, theme,
  mapWidth = 240, mapHeight = 150
}) => {
  ...
  const bounds = useMemo(() => {
    ...
    if (!b) return { 
        minX: -minPadding, maxX: minPadding, minY: -minPadding, maxY: minPadding, 
        width: minPadding * 2, height: minPadding * 2 
    };
  ...
  const toMapX = (x: number) => bounds ? (x - bounds.minX) * mapScale : 0;
  const toMapY = (y: number) => bounds ? (y - bounds.minY) * mapScale : 0;
  ...
  const handlePointer = (e: React.PointerEvent) => {
    if (!bounds || !canvasRef.current || !containerWidth || !containerHeight) return;
